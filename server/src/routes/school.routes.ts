...
const updateSchoolSchema = z.object({
  ...
  staffWorkStartTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),
  staffWorkEndTime: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).optional(),
  staffRequiredMinutes: z.number().int().min(60).max(1440).optional(),
  staffPartialDayMinutes: z.number().int().min(30).max(1439).optional(),
  staffWorkingDays: z.array(z.number().int().min(0).max(6)).min(1).max(7).optional(),
