...
const SCHOLARSHIP_OPTIONS = [
  ...
  },
  {
    ...
    sublabel: '50% discount applied to every fee item',
    color: 'violet',
...
// Scholarship badge config used in both the summary panel and the history list
...
export default function LearnerFeeConfigurator({ learner, user, onChanged }) {
  ...
  // Whether the scholarship type locks out per-item editing
  const scholarshipLock = form.scholarshipType === 'FULL' || form.scholarshipType === 'HALF';

  // ── Data loading ─────────────────────────────────────────────────────────

  const matchingStructure = useMemo(() => structures.find((s) =>
  ...
  const load = useCallback(async () => {
    const [configRes, structureRes] = await Promise.all([
  ...
  const adjustmentFor = (row) => {
    ...
    return form.adjustments.find((a) => a.feeTypeId === key || a.code === row.code) || {
      feeTypeId: key,
      code: row.code,
      name: row.name,
  ...
  const updateAdjustment = (row, patch) => {
    ...
    setForm((prev) => ({
      ...
      adjustments: [
        ...
        { ...current, feeTypeId: key, code: row.code, name: row.name, standardAmount: row.standardAmount || 0, mode: 'CUSTOM_AMOUNT', ...patch },
      ],
    }));
  };
  ...
  const setScholarshipType = (type) => {
    setForm((prev) => ({
      ...
      // Clear partial amount when switching away from PARTIAL_AMOUNT
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        scholarshipAmount: form.scholarshipType === 'PARTIAL_AMOUNT'
  ...
  // ── Summary totals (only meaningful for NONE / PARTIAL_AMOUNT) ────────────
  ...
  const partialAmountValid = form.scholarshipType !== 'PARTIAL_AMOUNT'
    || (Number(form.scholarshipAmount) > 0);
  const canSave = !saving && !!matchingStructure && canEdit && partialAmountValid;

  // ── Scholarship badge for the status cell ────────────────────────────────
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {canEdit && (
            <div className="space-y-4 p-4">
              ...
              <div>
                <p className="mb-2 text-[10px] font-black uppercase tracking-wide text-slate-500">Scholarship / discount type</p>
                ...
                {/* Partial amount input — only shown when PARTIAL_AMOUNT selected */}
                {form.scholarshipType === 'PARTIAL_AMOUNT' && (
                  <div className="mt-3 rounded-xl border-2 border-amber-200 bg-amber-50 p-3">
                    <label className="block">
                      ...
                      {form.scholarshipAmount && Number(form.scholarshipAmount) > 0 && (
                        <p className="mt-1 text-[10px] text-amber-600">
                          Student will be billed exactly {formatMoney(form.scholarshipAmount)} — the remainder is discounted.
                        </p>
                      )}
                      {(!form.scholarshipAmount || Number(form.scholarshipAmount) === 0) && (
                ...
                {/* Informational banners for FULL and HALF */}
                {form.scholarshipType === 'FULL' && (
                  <div className="mt-3 flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2">
                    ...
                    <p className="text-xs font-semibold text-red-700">
                      Full scholarship — this student will not be billed anything. Their invoice total will be KES 0.
                    </p>
                  </div>
                )}
                                ...
                                return (
                                  <div key={row.id} className="grid items-center gap-3 border-b border-slate-100 px-3 py-2 last:border-b-0 md:grid-cols-[minmax(220px,1fr)_100px_160px_110px]">
                                    ...
                                    <div>
                                      ...
                                      <input
                                        className="h-8 w-full rounded-md border border-slate-200 px-2 text-right text-sm font-black text-slate-900 disabled:bg-slate-100 disabled:text-slate-400"
                                        type="number"
                                        min="0"
                                        disabled={!checked}
                                        value={adj.value ?? ''}
        ...
        <aside className="space-y-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {/* Per-group subtotals (only relevant for NONE / PARTIAL_AMOUNT) */}
            ...
            {/* Editable total — hidden for FULL/HALF, shown for NONE/PARTIAL_AMOUNT */}
            {!scholarshipLock && (
              <div className="mt-4 rounded-xl border border-emerald-100 bg-emerald-50 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <input
                      className="mt-1 w-full bg-transparent text-2xl font-black text-emerald-700 outline-none"
                      type="number"
                      min="0"
                      value={selectedTotal}
            ...
            {/* FULL scholarship zero-bill indicator */}
            {form.scholarshipType === 'FULL' && (
          ...
          {preview && (
            <div className="rounded-2xl bg-slate-950 p-4 text-white shadow-sm">
              ...
              {preview.calculationSnapshot?.scholarshipType && preview.calculationSnapshot.scholarshipType !== 'NONE' && (
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div className="space-y-2">
              {configurations.map((cfg) => {
                ...
                return (
                  <div key={cfg.id} className="rounded-xl border border-slate-200 p-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-slate-500">
                          From {cfg.startTerm.replace('_', ' ')} {cfg.startAcademicYear}
                          {cfg.endTerm ? ` through ${cfg.endTerm.replace('_', ' ')} ${cfg.endAcademicYear}` : ' until amended'}
                        </p>
                    ...
                    <div className="mt-2 flex items-center justify-end gap-1">
                      {cfg.status !== 'REVOKED' && canEdit && (
                        <button title="Edit" onClick={() => edit(cfg)} className="rounded p-1.5 text-indigo-600 hover:bg-indigo-50">
                          <Edit3 size={14} />
                        </button>
                      )}
                      {!['APPROVED', 'REVOKED'].includes(cfg.status) && canApprove && (
                        <button title="Approve" onClick={() => approve(cfg.id)} className="rounded p-1.5 text-emerald-600 hover:bg-emerald-50">
                          <ShieldCheck size={15} />
                        </button>
                      )}
                      {cfg.status === 'APPROVED' && canApprove && (
                        <button title="Revoke" onClick={() => revoke(cfg.id)} className="rounded p-1.5 text-red-600 hover:bg-red-50">
                          <XCircle size={15} />
                        </button>
                      )}
