...
function QuickActions({ onNavigate }) {
  return (
    <div className="grid grid-cols-2 gap-4 h-full">
      {/* Academic Reports — dark gradient card */}
      <button
        type="button"
        onClick={() => onNavigate('parent-portal-results')}
        className="relative flex flex-col justify-between rounded-2xl p-5 text-left overflow-hidden bg-gradient-to-br from-emerald-600 to-teal-700 shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.99] transition-all group"
      >
        {/* decorative circle */}
        <span className="absolute -top-4 -right-4 w-24 h-24 rounded-full bg-white/10 pointer-events-none" />
        <span className="absolute bottom-2 right-2 w-12 h-12 rounded-full bg-white/5 pointer-events-none" />

        <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center mb-4 flex-shrink-0">
          <BarChart3 size={20} className="text-white" />
        </div>
        <div>
          <p className="text-sm font-bold text-white leading-tight">Academic Reports</p>
          <p className="text-[11px] text-emerald-100 mt-0.5 leading-tight">Track performance &amp; grades</p>
        </div>
      </button>

      {/* Attendance — light outlined card */}
      <button
        type="button"
        onClick={() => onNavigate('parent-portal-attendance')}
        className="relative flex flex-col justify-between rounded-2xl p-5 text-left overflow-hidden bg-white border-2 border-blue-100 shadow-sm hover:shadow-md hover:border-blue-300 hover:scale-[1.02] active:scale-[0.99] transition-all group"
      >
        {/* decorative accent bar */}
        <span className="absolute top-0 left-0 w-1 h-full rounded-l-2xl bg-gradient-to-b from-blue-400 to-indigo-500 pointer-events-none" />

        <div className="w-10 h-10 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center mb-4 flex-shrink-0">
          <ClipboardCheck size={20} className="text-blue-600" />
        </div>
        <div>
          <p className="text-sm font-bold text-gray-800 leading-tight">Attendance</p>
          <p className="text-[11px] text-gray-400 mt-0.5 leading-tight">Check daily records</p>
        </div>
      </button>
    </div>
  );
